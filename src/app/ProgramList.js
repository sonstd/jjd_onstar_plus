...
function parseHakbuXml(xmlText) {
  if (!xmlText) return null;
  if (typeof window === "undefined") return null; // SSR 가드
  const parser = new DOMParser();
  const xmlDoc = parser.parseFromString(xmlText, "text/xml");
  const daehakList = xmlDoc.querySelector("DAEHAK_LIST");
  if (!daehakList) return null;
  const daehaks = Array.from(daehakList.querySelectorAll("DAEHAK")).map((d) => ({
  ...
  return { DAEHAK_LIST: { DAEHAK: daehaks } };
}
...
function FilterPanel({ hakbuData, filters, setFilters, onReset }) {
  ...
  const selectedCollege = colleges.find((c) => c["@_name"] === filters.daehak);
      ...
      <div className={styles.filterSection}>
        ...
        <div className={styles.filterGrid}>
          ...
          <div className={styles.filterGroup}>
            ...
            >
              <option value="">전체</option>
              {(Array.isArray(departments) ? departments : [departments]).map((h) => (
...
// ── 메인 클라이언트 컴포넌트 ────────────────────────────
// props로 서버에서 미리 읽어온 데이터를 받아 첫 화면을 즉시 렌더링하고,
// 마운트 후 /api/all-prog 를 호출해 최신 데이터로 조용히 교체한다.
export default function ProgramList({ initialPrograms, hakbuXml }) {
  const [programs, setPrograms] = useState(initialPrograms || []);
  const [hakbuData, setHakbuData] = useState(null);
  const [error, setError] = useState(null);
  const [page, setPage] = useState(1);
  const [windowWidth, setWindowWidth] = useState(1280); // 서버와 동일한 초기값 (hydration mismatch 방지)
  ...
  // 반응형 width 추적 (마운트 후 실제 값으로 동기화 + resize 리스너)
  useEffect(() => {
    setWindowWidth(window.innerWidth); // 마운트 시점에 실제 값으로 한 번 동기화
  ...
  // 단과대/학부 XML 파싱 (DOMParser는 브라우저 전용이므로 클라이언트 마운트 후에만 실행)
  useEffect(() => {
    if (hakbuXml) {
      setHakbuData(parseHakbuXml(hakbuXml));
    }
  }, [hakbuXml]);

  // 마운트 후 최신 데이터로 조용히 갱신 (로딩 팝업 없음)
  useEffect(() => {
  ...
  const filtered = useMemo(() => {
    return programs.filter((p) => {
      if (filters.daehak) {
        if (p.TARGET_DAEHAK !== "" && p.TARGET_DAEHAK !== filters.daehak) return false;
      }
      if (filters.hakbu) {
        if (p.TARGET_HAKBU !== "" && p.TARGET_HAKBU !== filters.hakbu) return false;
      }
          ...
          <>
            ...
            {filtered.length === 0 && (
              <div className={styles.emptyBox}>
                ...
                <p className={styles.emptyDesc}>필터를 변경하거나 초기화해 보세요.</p>
      ...
      <footer className={styles.footer}>
        <div className={styles.footerInner}>
          ...
          <p className={styles.footerCopyright}>
            Copyright © 2026 Son Ji Ho. All rights reserved.
          </p>
        </div>
      </footer>
    </main>
